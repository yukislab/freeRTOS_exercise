...
//任务1：读按键键值、key0按下1位置一，key1按下2位置一
void task1(void* pvParameters)
{
	uint8_t key=0;
	while(1)
...
void task2(void* pvParameters)
{
	uint32_t rev=0,event_bit=0;//用于传出任务通知值
	while(1)
	{
        xTaskNotifyWait( 0, 0xFFFFFFFF, &rev, portMAX_DELAY );
		if(rev&EVENTBIT_0)//相与为1，意味着bit0置1
		{	event_bit|=EVENTBIT_0;//保存bit0的1
			printf("接收%x\n",event_bit);
			event_bit=0;
		}
		if(rev&EVENTBIT_1)//相与为1，意味着bit1置1
		{	event_bit|=EVENTBIT_1;//保存bit0的1
			printf("接收%x\n",event_bit);
			event_bit=0;
		}
	}
